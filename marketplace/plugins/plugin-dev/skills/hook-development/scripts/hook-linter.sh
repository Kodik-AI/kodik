...
check_script() {
  ...
  if grep -E '^[^#]*/home/|^[^#]*/usr/|^[^#]*/opt/' "$script" | grep -q .; then
    ...
    echo "   Use \$CLAUDE_PROJECT_DIR or \$KODIK_PLUGIN_ROOT"
    ((warnings++))
  fi

  # Check 8: Uses KODIK_PLUGIN_ROOT
  if ! grep -q "KODIK_PLUGIN_ROOT\|CLAUDE_PROJECT_DIR" "$script"; then
    echo "💡 Tip: Use \$KODIK_PLUGIN_ROOT for plugin-relative paths"
  fi
